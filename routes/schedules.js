...
router.get('/:scheduleId', authenticationEnsurer, param('scheduleId').isUUID(), async (req, res, next) => {
  const errors = validationResult(req);

    if (!errors.isEmpty()) {
      return next(new Error('無効な URL です'));
    }
  ...
  if (schedule) {
    ...
    // 閲覧ユーザと出欠に基づくユーザからユーザ Map (キー:ユーザ ID, 値:ユーザ) を作る
